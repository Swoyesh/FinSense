...
def visualize_forecast(monthly_data, forecasts, model_summary):
    ...
    for i, category in enumerate(exp_cate):
        ...
        if category in forecasts:
            forecast_month = monthly_data.index[-1] + pd.DateOffset(months=1)
            ax.scatter(forecast_month, forecasts[category], color='red', s=100, zorder=5, label='Forecast')

            if category in model_summary:
                lower_bound = model_summary[category].get('confidence_lower', 0)
                upper_bound = model_summary[category].get('confidence_upper', forecasts[category])
                ax.fill_between(
                    [forecast_month - pd.DateOffset(days=15), forecast_month + pd.DateOffset(days=15)],
