...
"""
    ...
    async def sql_query_answer(self, user_id: int, query: str):
        ...
        # Detect if user wants detailed breakdown
        wants_details = any(keyword in query_lower for keyword in [
            'breakdown', 'break down', 'break it down', 'detail', 'details', 
            'list', 'show', 'show me', 'transactions', 'what did i', 
            'where did i', 'itemize', 'further', 'more information',
            'elaborate', 'expand'
        ])
        
        # Extract category from query
        categories = [
            'food', 'groceries', 'dining', 'restaurant', 'transport', 
            'entertainment', 'shopping', 'utilities', 'healthcare', 
            'education', 'subscription', 'income', 'salary'
        ]
        ...
        print(f"🔍 Query analysis: wants_details={wants_details}, category={mentioned_category}")
    ...
    async def generate_sql_query(self, user_id: int, query: str, wants_details: bool = False, category: str = None):
        """
        ...
        Args:
            ...
            wants_details: Whether user wants detailed breakdown (optional)
            category: Specific category mentioned in query (optional)
        """
        
        # Modify the prompt based on wants_details
        detail_instruction = ""
        if wants_details:
            detail_instruction = """
    IMPORTANT: User wants DETAILED breakdown with individual transactions.
    - SELECT date_time, description, amount, category, balance
    - ORDER BY date_time DESC
    - LIMIT 50 for readability
    - Show individual rows, NOT aggregated sums
    """
    ...
    Q: "Break down my food expenses last month" (DETAILED)
    SQL: SELECT date_time, description, amount, category, balance FROM transactions WHERE user_id = {user_id} AND category ILIKE '%food%' AND type = 'expense' AND CAST(date_time AS DATE) >= DATE_TRUNC('month', CURRENT_DATE - INTERVAL '1 month')::date AND CAST(date_time AS DATE) < DATE_TRUNC('month', CURRENT_DATE)::date ORDER BY date_time DESC LIMIT 50
    ...
    Q: "Show my travel expenses this month" (DETAILED)
...
if __name__ == "__main__":
    async def main():
        tsql = test_sql(db_config=data_db, groq_api_key=GROQ_API_KEY)
